...
bool isPrime(int number) {
  // if sqrt returns whole number that can be divided by itself, then its not a prime
...
int display(int inputPrime, int inputRow) {
  ...
  // as long as the number of primes is less than the total primes, continue printing
  while (ct < inputPrime) {
    if (isPrime(sequenceNum)) {
      ...
      ct++;
      rowCt++;
      // if the number of primes in a row is equal to the input row, start a new line
      if (rowCt == inputRow) {
        cout << endl;
        rowCt = 0;
